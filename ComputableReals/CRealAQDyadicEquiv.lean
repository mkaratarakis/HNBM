...
theorem abs_toRat_toDyadic_sub_le (q : ℚ) (n : Nat) :
    |(Rat.toDyadic q n).toRat - q| ≤ (1 : ℚ) / (2 ^ n) :=
  -- `DyadicApprox.approxRat` unfolds to `Rat.toDyadic`, so this is exactly
  -- the bound already proved in `CRealPreDyadic`.
  Computable.DyadicApprox.abs_toRat_approxRat_sub_le q n
...
def repOfPre (x : CReal.Pre) : CRealRep Dyadic where
  ...
  is_regular := by
    ...
    have h_pow_mon : (1 : ℚ) / (2 ^ (m + 2)) ≤ (1 : ℚ) / (2 ^ (n + 2)) := by
      have hnm' : n + 2 ≤ m + 2 := Nat.add_le_add_right hnm 2
      have hpow : (2 : ℚ) ^ (n + 2) ≤ (2 : ℚ) ^ (m + 2) := pow_le_pow_right₀ (by norm_num) hnm'
    ...
    have h3 : (1 : ℚ) / (2 ^ (n + 2)) + (1 : ℚ) / (2 ^ (n + 2)) + (1 : ℚ) / (2 ^ (n + 2))
      ...
      have hquarter : (1 : ℚ) / (2 ^ (n + 2)) = (1 : ℚ) / (2 ^ n) / 4 := by
        rw [pow_add]; ring
      rw [hquarter]
      linarith [hpos]
