...
/-! ### Embedding of spec pre-reals into representatives -/

/--
Round a ℚ-pre-real into a representative over `AQ` (with slack `+2`).

This is the backend-agnostic version of the dyadic construction in `CRealAQDyadicEquiv.lean`.
It is the key bridge that lets us lift algorithms proved on the ℚ-spec model to any
`ApproxRationals` backend.
-/
def ofPre (x : CReal.Pre) : CRealRep AQ where
  approx := fun n => ApproxRationals.approxRat (x.approx (n + 2)) (n + 2)
  is_regular := by
    intro n m hnm
    -- triangle inequality around the original rational approximants
    set a : ℚ := ApproxRationals.toRat (AQ := AQ)
      (ApproxRationals.approxRat (AQ := AQ) (x.approx (n + 2)) (n + 2))
    set b : ℚ := ApproxRationals.toRat (AQ := AQ)
      (ApproxRationals.approxRat (AQ := AQ) (x.approx (m + 2)) (m + 2))
    set xn : ℚ := x.approx (n + 2)
    set xm : ℚ := x.approx (m + 2)
    have hn_round : |a - xn| ≤ (1 : ℚ) / (2 ^ (n + 2)) := by
      -- from the `approxRat` specification
      simpa [a, xn] using
    ...
    have h_pow_mon : (1 : ℚ) / (2 ^ (m + 2)) ≤ (1 : ℚ) / (2 ^ (n + 2)) := by
      have hnm' : n + 2 ≤ m + 2 := Nat.add_le_add_right hnm 2
      have hpow : (2 : ℚ) ^ (n + 2) ≤ (2 : ℚ) ^ (m + 2) := (pow_le_pow_iff_right₀ rfl).mpr hnm'
    ...
    have h3 :
        (1 : ℚ) / (2 ^ (n + 2)) + (1 : ℚ) / (2 ^ (n + 2)) + (1 : ℚ) / (2 ^ (n + 2))
          ≤ (1 : ℚ) / (2 ^ n) := by
      -- `3 * 2^{-(n+2)} ≤ 2^{-n}`
      have h2n_pos : (0 : ℚ) < (2 : ℚ) ^ n := pow_pos (by norm_num) n
      field_simp
      ring_nf
      nlinarith [h2n_pos]
...
/-- The rounded representative denotes the original ℚ-pre-real. -/
theorem toPre_ofPre_equiv (x : CReal.Pre) :
    CReal.Pre.Equiv (ofPre (AQ := AQ) x).toPre x := by
  intro n
  -- compare at index `n+1`
  set a : ℚ := (ofPre (AQ := AQ) x).toPre.approx (n + 1)
  set b : ℚ := x.approx (n + 1)
  set c : ℚ := x.approx (n + 3)
  have h_round : |a - c| ≤ (1 : ℚ) / (2 ^ (n + 3)) := by
    -- this is the rounding error at index `n+3`
    simpa [ofPre, CRealRep.toPre, a, c] using
      (ApproxRationals.abs_toRat_approxRat_sub_le (AQ := AQ) (q := x.approx (n + 3)) (n := n + 3))
  have h_reg : |c - b| ≤ (1 : ℚ) / (2 ^ (n + 1)) := by
    have := x.is_regular (n + 1) (n + 3) (by omega)
    -- `is_regular` gives the bound in the opposite subtraction order.
    simpa [abs_sub_comm, b, c] using this
  have h_tri : |a - b| ≤ |a - c| + |c - b| := by
    simpa [sub_eq_add_neg, add_assoc, add_left_comm, add_comm] using (abs_sub_le a c b)
  have h_close :
      (1 : ℚ) / (2 ^ (n + 3)) + (1 : ℚ) / (2 ^ (n + 1)) ≤ (1 : ℚ) / (2 ^ n) := by
    -- Rewrite everything as a multiple of `(1/2^n)` and use `5/8 ≤ 1`.
    have h1 : (1 : ℚ) / (2 ^ (n + 3)) = ((1 : ℚ) / (2 ^ n)) / 8 := by
      -- `2^(n+3) = 2^n * 8`
      simp [pow_succ, div_eq_mul_inv]
      ring_nf
    have h2 : (1 : ℚ) / (2 ^ (n + 1)) = ((1 : ℚ) / (2 ^ n)) / 2 := by
      -- `2^(n+1) = 2^n * 2`
      simp [pow_succ, div_eq_mul_inv]
      ring_nf
    set A : ℚ := (1 : ℚ) / (2 ^ n)
    have hA : 0 ≤ A := by positivity
    have hcoeff : (1 : ℚ) / 8 + (1 : ℚ) / 2 ≤ (1 : ℚ) := by norm_num
    calc
      (1 : ℚ) / (2 ^ (n + 3)) + (1 : ℚ) / (2 ^ (n + 1))
          = A / 8 + A / 2 := by simp [A, h1, h2]
      _ = A * ((1 : ℚ) / 8 + (1 : ℚ) / 2) := by
          simp [div_eq_mul_inv, mul_add]
      _ ≤ A * 1 := by
          exact mul_le_mul_of_nonneg_left hcoeff hA
      _ = (1 : ℚ) / (2 ^ n) := by simp [A]
  have : |a - b| ≤ (1 : ℚ) / (2 ^ n) := (h_tri.trans (by linarith [h_round, h_reg])).trans h_close
  simpa [CReal.Pre.Equiv, a, b] using this

/-- `ofPre` respects the spec equivalence relation. -/
theorem ofPre_respects_equiv {x y : CReal.Pre} (hxy : CReal.Pre.Equiv x y) :
    CReal.Pre.Equiv (ofPre (AQ := AQ) x).toPre (ofPre (AQ := AQ) y).toPre := by
  have hx : CReal.Pre.Equiv (ofPre (AQ := AQ) x).toPre x := toPre_ofPre_equiv (AQ := AQ) x
  have hy : CReal.Pre.Equiv (ofPre (AQ := AQ) y).toPre y := toPre_ofPre_equiv (AQ := AQ) y
  -- (ofPre x) ≈ x ≈ y ≈ (ofPre y)
  exact CReal.Pre.equiv_trans (CReal.Pre.equiv_trans hx hxy) (CReal.Pre.equiv_symm hy)
